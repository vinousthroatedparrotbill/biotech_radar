...
- **★밸류 드라이버 판정 — 정량 기준(현재 EV 앵커)**: 각 자산의
  **peak OP = peak sales × OPM(성숙 바이오 ~40% 기본 가정, 근거 있으면 조정)**을 계산하고,
  이 **peak OP가 현재 EV의 50% 이상**이면 밸류 드라이버로 판정하라. 기준은 미래 SOTP 비중이
  아니라 **"현재 EV 대비"** 다(= 현 EV ≤ 2× 그 자산 peak OP). risk-adjust(PoS) 적용 시 명시.
  표기: "★밸류 드라이버: <자산명> — peak $XB × OPM 40% = peak OP $YB ≥ 현재 EV $ZB × 50%".
- **정성 교차검증(정량만으로 끝내지 말 것)**: 그 자산이 실제 드라이버인지 ① 최근 뉴스 플로우
  (회사·시장이 이 자산에 집중하나 — search_news_by_query/fetch_recent_news) ② 주가 반응
  (이 자산 관련 이벤트에 주가가 실제로 반응했나 — get_realtime_quote 수익률+뉴스 매칭)
  ③ peer/유사 자산의 시장 평가로 **교차 확인**하라. 정량(peak OP)과 정성(시장 관심)이 엇갈리면
  그 **불일치를 반드시 명시**(예: "정량상 A가 드라이버지만 주가·뉴스는 B에 반응 — 시장은 아직 A 미반영").
- 밸류 드라이버 자산은 **§4 경쟁분석·§6 카탈리스트·§8 리스크에서 최우선·심층 대상**(얕게 넘기지 말 것).
- 도구 조사로 컨센서스 못 찾으면 "공개 컨센서스 부족 — 자체 가정 명시" 후 추정.

## 4) 경쟁 파이프라인 — head-to-head 차별 분석 ★ 가장 중요 ★
**§3에서 판정된 밸류 드라이버 자산은 절대 누락 금지 — 여기서 최우선·최심층으로 다룬다.**
각 자산(특히 밸류 드라이버)에 대해 같은 분자 타깃 / 같은 기전 / 같은 적응증의 경쟁 약물을
**web_search로 회사명·자산명까지 특정해 조사**하라 — 대형사뿐 아니라 소형·비주류·신규상장·비상장까지.
**밸류 드라이버 자산은 "<타깃> antibody companies", "<타깃> <적응증> competitor pipeline" 같은
전용 검색을 돌려 같은 분자 타깃 경쟁사를 반드시 실명(회사·자산)으로 나열**하라(같은 타깃이면
시총 무관 — 예: anti-CD122면 Forte Biosciences(FBRX/FB102)까지). 학습지식만으론 틈새를 놓치니
반드시 검색으로 발굴. peers(유사 아이디어) 결과와도 정합하게.
