...
def cloud_check(sample: int = 15) -> dict:
    ...
    def _probe(name, fn):
        ...
        try:
            res = fn()
            res["elapsed_s"] = round(_t.time() - t0, 1)
            out[name] = res
        except Exception as e:
    ...
    _probe("finviz_universe", _finviz)
    ...
    def _yf():
        ...
        if df is not None and "Ticker" in df.columns:
            ...
            tks = ["VRTX", "REGN", "GILD", "AMGN", "BIIB", "MRNA", "ILMN",
                   "ARWR", "EXEL", "INCY"][:sample]
        snap = compute_snapshot(tks)
        got = int(len(snap)) if snap is not None else 0
        r = {"ok": got > 0, "requested": len(tks), "got": got}
        if got < len(tks):
            r["note"] = "일부/전체 누락 — Yahoo 429/차단 가능성"
        return r
    _probe("yfinance_highlow", _yf)

    def _news():
        from news import fetch_combined
        items = fetch_combined("VRTX", "Vertex Pharmaceuticals", 21)
        return {"ok": len(items) > 0, "titles": len(items)}
    _probe("news", _news)
    ...
    def _mp():
        ...
        ports = pf.list_all()
        if not ports:
            return {"ok": True, "note": "포트폴리오 없음(스킵)", "portfolios": 0}
        s = pf.summary(ports[0]["id"])
        hs = (s or {}).get("holdings") or []
        priced = sum(1 for h in hs if (h.get("curr_price") or 0) > 0)
        return {"ok": priced > 0 or len(hs) == 0, "holdings": len(hs),
                "priced": priced, "return_pct": (s or {}).get("return_pct")}
    _probe("mp_prices", _mp)

    def _tg():
        import os as _os
        import requests as _rq
        tok = (_os.environ.get("TELEGRAM_BOT_TOKEN") or "").strip()
        if not tok:
            return {"ok": False, "error": "TELEGRAM_BOT_TOKEN 미설정"}
        r = _rq.get(f"https://api.telegram.org/bot{tok}/getMe", timeout=15).json()
        return {"ok": bool(r.get("ok")), "bot": (r.get("result") or {}).get("username")}
    _probe("telegram", _tg)
    ...
    def _claude():
        ...
        r = c.messages.create(model="claude-opus-4-8", max_tokens=10,
                              messages=[{"role": "user", "content": "ok"}])
        return {"ok": True, "in": r.usage.input_tokens, "out": r.usage.output_tokens}
    _probe("claude", _claude)

    out["verdict"] = "ALL_OK" if all(v.get("ok") for v in out.values()
                                     if isinstance(v, dict)) else "SOME_FAILED"
    return out
